...
- **ERROR "Incorrect error output configuration… appear to be error handlers but are in main[0]"** on a fan-out where one target is a Respond to Webhook or Send Email node, or has *error / fail / catch / exception* in its name. If that node belongs on the success path, the fan-out is correct. **Don't apply the suggested fix.** Moving Respond to Webhook onto `main[1]` means the webhook only answers when the upstream node fails. ([n8n-mcp#1111](https://github.com/czlonkowski/n8n-mcp/issues/1111))
